...
def test_vid():
    ...
    for seq in seq_list:
        ...
        print(f'序列 psnr {sum(seq_psnr)/len(seq_psnr)} ssim {sum(seq_ssim)/len(seq_ssim)} 序列 psnr Y {sum(seq_psnr_Y)/len(seq_psnr_Y)} ssim Y {sum(seq_ssim_Y)/len(seq_ssim_Y)}')
        print(f'目前平均 psnr {sum(avg_psnr)/len(avg_psnr)} ssim {sum(avg_ssim)/len(avg_ssim)} psnr Y {sum(avg_psnr_Y)/len(avg_psnr_Y)} ssim Y {sum(avg_ssim_Y)/len(avg_ssim_Y)} ')
