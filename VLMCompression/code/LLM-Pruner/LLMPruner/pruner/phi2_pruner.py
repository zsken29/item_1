    ...
    def __call__(self, group, ch_groups=1, consecutive_groups=1):
        ...
        for dep, idxs in group:
            ...
            if prune_fn in [hf_attention_pruner.prune_out_channels]:
                salience = {}
                for sub_layer in [layer.dense, layer.q_proj, layer.k_proj, layer.v_proj]:
            ...
            # Attention
                ...
                local_norm = 0
                for sub_layer in [layer.dense]: #linear out channel, first dim in linear.weight
                    if self.taylor == 'vectorize':
