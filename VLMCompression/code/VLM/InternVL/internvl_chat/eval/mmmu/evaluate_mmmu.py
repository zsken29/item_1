...
def post_process(pred, option):
    ...
    if len(pred) == 1 and pred in option_candidate:
        return pred
    elif len(pred) == 1 and pred not in option_candidate:
        pred = random.choice(option_candidate)
        return pred
...
if __name__ == '__main__':
    ...
    tokenizer,model = load_pruned_model_devel(args.model_path, args.pruned_model,args.mm,args.lora,torch_dtype=torch.bfloat16)
    model.cuda()
