...
from internvl.model.internvl_chat import (InternVisionConfig,
                                          ...
                                          InternVLChatConfig,
                                          InternVLChatModel)
from internvl.model.internvl_chat.builder import load_pruned_model,load_distill_model
...
from trainer import InternvlDistillationTrainer
...
class ModelArguments:
    ...
    distill: float = field(
        default=None,
        metadata={'help': 'distillation rate for the distillation model.'}
    )
    pruned_model_path: Optional[str] = field(
    default=None,
    metadata={'help': 'Path to the pruned model.'}
    )
...
def main():
    print("++++++++++++++++The world size is", torch.cuda.device_count())
    print("++++++++++++++++CUDA_VISIBLE_DEVICES is", os.environ['CUDA_VISIBLE_DEVICES'])
    import subprocess

    def print_visible_gpus():
        try:
    ...
    if model_args.distill:
        tokenizer, model, teacher_model = load_distill_model(model_path=model_args.model_name_or_path,pruned_model_path=model_args.pruned_model_path, torch_dtype=torch.bfloat16)
        model.get_teacher(teacher_model)
    else:
        tokenizer, model = load_pruned_model(model_args.model_name_or_path,model_args.pruned_model_path, torch_dtype=torch.bfloat16)
    ...
    if model_args.distill:
        model.teacher_model.img_context_token_id = img_context_token_id
    ...
    if model_args.distill:
        model.teacher_model.num_image_token = model.num_image_token
    ...
    if model_args.distill:
        model.teacher_model.language_model.config.use_cache = False
    ...
    train_dataset = build_datasets(
        data_args, tokenizer, tcs_loader, model.num_image_token, group_by_length=training_args.group_by_length,
    ...
    if model_args.distill:
        training_args.dist_alpha = model_args.distill
        trainer = InternvlDistillationTrainer(
        model=model,
        args=training_args,
