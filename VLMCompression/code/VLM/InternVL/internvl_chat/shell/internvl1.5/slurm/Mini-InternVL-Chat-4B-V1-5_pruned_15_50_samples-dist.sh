...
#SBATCH --output=./slurm-logs/Mini-InternVL-Chat-4B-V1-5_pruned_15_50_samples-dist-out.%j
#SBATCH --error=./slurm-errors/Mini-InternVL-Chat-4B-V1-5_pruned_15_50_samples-dist-err.%j
...
python -m torch.distributed.run \
  ...
  --pruned_model_path "${REPO_ROOT}/ShortGPT/prune_log/Mini-InternVL-Chat-4B-V1-5_pruned_15_50_samples/pruned_model.bin" \
  --conv_style "phi3-chat" \
  --output_dir '${REPO_ROOT}/VLM/InternVL/internvl_chat/ckpt/dist-15/' \
  ...
  2>&1 | tee -a "${REPO_ROOT}/VLM/InternVL/internvl_chat/ckpt/dist-15/training_log.txt"
'